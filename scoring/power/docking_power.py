...
def usage():
    ...
    print("-s or --score: specify the directory containing your scoring files(e.g. 'XXXX_score.dat'). \
          Remember the 1st column name is #code and the 2nd column name is score. Supported file separators are comma(,),\
          tabs(\\t) and space character( )")
    ...
    print("-w or --withligand: include the Ligand pose or exclude the Ligand pose")
    print("\nExample: python docking_power.py -c /srv/nfs4/Mercury/wangzh/dataset/CASF-2016/power_scoring/CoreSet.dat -s docking-2016/ -r /srv/nfs4/Mercury/wangzh/dataset/CASF-2016/decoys_docking/ -p 'negative' -l 2 -w 'no' -o 'myscore' > MyDockingPower.out")


try:
    options, args = getopt.getopt(sys.argv[1:], "hc:s:r:p:l:w:o:", ["help", "coreset=", "score=", "rmsd=", "prefer=", "limit=", "withligand=", "output"])
...
for name, value in options:
    ...
    if name in ("-c", "--coreset"):
        ...
        f.close()
        f1.close()
        aa = pd.read_csv('cstemp', sep='[,,\t, ]+', header=0, engine='python')
...
dockresults = pd.DataFrame(index=range(1, len(pdb) + 1),
                           columns=['code', 'Rank1', 'RMSD1', 'Rank2', 'RMSD2', 'Rank3', 'RMSD3'])
tmp = 1
if fav == 'positive':
    for i in pdb:
        rmsddf = pd.read_csv(rmsdfile + '/' + str(i) + '_rmsd.dat', sep='[,, ,\t]+', engine='python')
        scoredf = pd.read_csv(scorefile + '/' + str(i) + '_score.dat', sep='[,, ,\t]+', engine='python')
        
        if with_lig == 'no':
            rmsddf = rmsddf.iloc[:-1, :]
            scoredf = scoredf.iloc[:-1, :]

        rmsddf = rmsddf.sort_values(by=rmsddf.columns[0])
        scoredf = scoredf.sort_values(by=scoredf.columns[0])
        ...
        for s in np.arange(2, 11):
            ...
            if float(sptemp.shape[0]) >= 5:
                spearman_corr = sptemp[['rmsd', 'score']].corr(method='spearman')
                spearman_value = spearman_corr.loc['rmsd', 'score']
                sp.loc[i, 'spearman'] = spearman_value
            else:
                continue
    ...
    for i in pdb:
        ...
        dockresults.loc[tmp, 'Rank1'] = ''.join(dfsorted[0:1]['#code'])
        dockresults.loc[tmp, 'RMSD1'] = float(dfsorted.iloc[0]['rmsd'])
        dockresults.loc[tmp, 'Rank2'] = ''.join(dfsorted[1:2]['#code'])
        dockresults.loc[tmp, 'RMSD2'] = float(dfsorted.iloc[1]['rmsd'])
        dockresults.loc[tmp, 'Rank3'] = ''.join(dfsorted[2:3]['#code'])
        dockresults.loc[tmp, 'RMSD3'] = float(dfsorted.iloc[2]['rmsd'])
        dockresults.loc[tmp, 'code'] = i
...
# Print the output of docking power evaluation
