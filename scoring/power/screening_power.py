...
def usage():
    ...
    print("-s or --score: specify the directory containing your scoring files(e.g. 'XXXX_score.dat'). Remember the 1st column name is #code_ligand_num and the 2nd column name is score. Supported file separators are comma(,), tabs(\\t) and space character( )")
    print("-p or --prefer: input 'negative' or 'positive' string, depend on your scoring function preference")
    print("-o or --output: input the prefix of output result files. Default is My_Forward_Screening_Power")
    print("-t or --target: specify the location of 'TargetInfo.dat' in the CASF-2016 package")
    print("-h or --help: print help message")
    print("\nExample: python screening_power.py -c ~/dataset/CASF-2016/power_screening/CoreSet.dat -s ./screening-2016/ -t ~/dataset/CASF-2016/power_screening/TargetInfo.dat -p 'negative' -o 'Myscore' > MyForwardScreeningPower.out")

try:
    options, args = getopt.getopt(sys.argv[1:], "hc:s:t:p:o:", ["help", "coreset=", "score=", "target=", "prefer=", "output="])
...
if fav == 'positive':
    for i in targetlst:
        scoredf = pd.read_csv(scorefile + '/' + str(i) + '_score.dat', sep='[ ,_,\t]+', engine='python')
        ...
        for m in range(1, t10 + 1):
            forwardf.loc[tmp, m] = ''.join(dfsorted.iloc[m - 1:m].index.tolist())
        ...
        for name, j in dic.items():
            lst = list(dfsorted.iloc[0:j].index)
...
top10success = dec(float(Top10['success'].sum()) / float(Top10.shape[0]), 3) * 100
...
# Print the output of forward screening power evaluation
tmplen = len(Top1)
