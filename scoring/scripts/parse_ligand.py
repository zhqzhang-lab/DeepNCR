...
#             # self.poses_file_names.append(pose_basename + "-" + str(name_index))
#         # for line in lines:
#         #     if line.startswith('MODEL'):
#         #         self.poses_file_names.append(pose_basename + "-" + str(line.split()[1]))
#         for line in lines:
#             if line.startswith("MODEL"):
#                 self.poses_file_names.append(f"{pose_basename}-{line[5:].strip()}")
...
class Ligand():
    # class-level caches for JSON files to avoid reloading 50w times
    _atomtype_mapping = None
    _covalent_radii_dict = None
    _vdw_radii_dict = None
    ...
    def __init__(self, poses_file: str=None):
        ...
        # load JSON files once (class-level)
        if Ligand._atomtype_mapping is None:
            with open(os.path.join(_current_dpath, "atomtype_mapping.json")) as f:
                Ligand._atomtype_mapping = json.load(f)
            with open(os.path.join(_current_dpath, "covalent_radii_dict.json")) as f:
                Ligand._covalent_radii_dict = json.load(f)
            with open(os.path.join(_current_dpath, "vdw_radii_dict.json")) as f:
                Ligand._vdw_radii_dict = json.load(f)

        # instance references (fast access)
        self.atomtype_mapping = Ligand._atomtype_mapping
        self.covalent_radii_dict = Ligand._covalent_radii_dict
        self.vdw_radii_dict = Ligand._vdw_radii_dict
    ...
    def _get_poses_fpath(self):
        ...
        # faster read: read once and splitlines (avoid .strip() per line)
    ...
    def _get_xyz(self, num, _lines):
        ...
        # filter only atom-like lines (keep original behavior); use startswith tuple check
        atom_lines = [x for x in _lines if x[:4] == "ATOM" or x[:4] == "HETA"]

        # parse coordinates using list comprehension (faster than repeated np.c_)
        coords = []
        for line in atom_lines:
            # slice positions kept same as original
            x = float(line[30:38].strip())
            y = float(line[38:46].strip())
            z = float(line[46:54].strip())
            coords.append((x, y, z))
        ...
        if coords:
            all_arr = np.array(coords, dtype=np.float32)
        else:
            all_arr = np.zeros((0,3), dtype=np.float32)

        # heavy atom selection: fallback to zip if lengths mismatch (preserve robustness)
        ha_list = []
        if len(self.lig_all_atoms_xs_types) == all_arr.shape[0]:
            # direct mask
            mask = [k != "dummy" for k in self.lig_all_atoms_xs_types]
            if any(mask):
                ha_arr = all_arr[np.array(mask)]
            else:
                ha_arr = np.zeros((0,3), dtype=np.float32)
        else:
            # fallback: zip
            for k, v in zip(self.lig_all_atoms_xs_types, all_arr):
                if k != "dummy":
                    ha_list.append(v)
            ha_arr = np.array(ha_list, dtype=np.float32) if ha_list else np.zeros((0,3), dtype=np.float32)

        ha_xyz = torch.from_numpy(ha_arr).to(torch.float32)
        all_xyz = torch.from_numpy(all_arr).to(torch.float32)

        # ensure ligands storage is preallocated (should be allocated in _parse_frame)
        # but guard in case it's not
        if self.init_lig_heavy_atoms_xyz.numel() == 0 or num >= self.init_lig_heavy_atoms_xyz.size(0):
            # attempt to expand storage: allocate minimal shape (number_of_poses, current_ha_count,3)
            prev_nposes = 0 if self.init_lig_heavy_atoms_xyz.numel() == 0 else self.init_lig_heavy_atoms_xyz.size(0)
            new_nposes = max(self.number_of_poses, num+1)
            ha_count = ha_xyz.size(0)
            self.init_lig_heavy_atoms_xyz = torch.zeros((new_nposes, ha_count, 3), dtype=torch.float32)
        if self.lig_all_atoms_xyz.numel() == 0 or num >= self.lig_all_atoms_xyz.size(0):
            prev_nposes = 0 if self.lig_all_atoms_xyz.numel() == 0 else self.lig_all_atoms_xyz.size(0)
            new_nposes = max(self.number_of_poses, num+1)
            atom_count = all_xyz.size(0)
            self.lig_all_atoms_xyz = torch.zeros((new_nposes, atom_count, 3), dtype=torch.float32)

        # compute ligand center and assign
        ligand_center = torch.mean(ha_xyz, dim=0) if ha_xyz.numel() else torch.zeros(3)
        # ensure ligand_center tensor has right shape
        if self.ligand_center.numel() == 0:
            self.ligand_center = torch.zeros((self.number_of_poses, 3), dtype=torch.float32)
        if num >= self.ligand_center.size(0):
            # expand if necessary
            tmp = torch.zeros((self.number_of_poses, 3), dtype=torch.float32)
            tmp[:self.ligand_center.size(0)] = self.ligand_center
            self.ligand_center = tmp
        self.ligand_center[num] = ligand_center
    ...
    def _parse_frame(self, _lines):
        lines = [x for x in _lines]  # already split in _get_poses_fpath
        ...
        for line in root_lines:
            ...
            atom_xyz = np.array((x, y, z), dtype=np.float32)
            lig_all_atoms_xyz.append(atom_xyz)
        ...
        for num, start_num in enumerate(branch_start_numbers):
            ...
            if num == number_of_branch - 1:
                _the_branch_lines = [x for x in lines[start_num:] if
                                     x.startswith("ATOM") or x.startswith("HETATM")]

            else:
                end_num = branch_start_numbers[num + 1]
                _the_branch_lines = [x for x in lines[start_num:end_num] if
                                     x.startswith("ATOM") or x.startswith("HETATM")]

            each_frame_all_atoms_index = []
            each_frame_heavy_atoms_index = []

            # reset per-branch counter
            number_of_heavy_atom = 0
        ...
        # convert to torch
        init_lig_heavy_atoms_xyz = torch.from_numpy(np.array(init_lig_heavy_atoms_xyz)).to(torch.float32) if init_lig_heavy_atoms_xyz else torch.zeros((0,3), dtype=torch.float32)
        lig_all_atoms_xyz = torch.from_numpy(np.array(lig_all_atoms_xyz)).to(torch.float32) if lig_all_atoms_xyz else torch.zeros((0,3), dtype=torch.float32)

        # preallocate storage for all poses
        self.init_lig_heavy_atoms_xyz = torch.zeros((self.number_of_poses, init_lig_heavy_atoms_xyz.shape[0], 3), dtype=torch.float32)
        self.lig_all_atoms_xyz = torch.zeros((self.number_of_poses, lig_all_atoms_xyz.shape[0], 3), dtype=torch.float32)
        
        self.init_lig_heavy_atoms_xyz[0] = init_lig_heavy_atoms_xyz
        self.lig_all_atoms_xyz[0] = lig_all_atoms_xyz

        ligand_center = torch.mean(init_lig_heavy_atoms_xyz, dim=0) if init_lig_heavy_atoms_xyz.numel() else torch.zeros(3)
        self.ligand_center = torch.zeros((self.number_of_poses, 3), dtype=torch.float32)
       
        self.ligand_center[0] = ligand_center

        self.lig_all_atoms_indices = [x for x in range(lig_all_atoms_xyz.shape[0])]
    ...
    def update_ligand_bonded_information(self):
        ...
        self.number_of_frames = len(self.frame_heavy_atoms_index_list)
        self.number_of_heavy_atoms = 0 if self.init_lig_heavy_atoms_xyz.numel() == 0 else self.init_lig_heavy_atoms_xyz.size(1)
    ...
    def _lig_carbon_is_hydrophobic(self, carbon_index, candidate_neighbors_indices):
        ...
        if carbon_index in self.lig_carbon_is_hydrophobic_dict.keys():
            ...
            pos_carbon = self.lig_all_atoms_xyz[0][carbon_index]
            for candi_neighb_index in candidate_neighbors_indices:
                if carbon_index == candi_neighb_index:
                    continue
                else:
                    candi_pos = self.lig_all_atoms_xyz[0][candi_neighb_index]
                    candi_d = torch.sqrt(torch.sum((pos_carbon - candi_pos) ** 2))
                    if candi_d <= float(self.covalent_radii_dict[self.lig_all_atoms_ad4_types[carbon_index]]) + \
                            float(self.covalent_radii_dict[self.lig_all_atoms_ad4_types[candi_neighb_index]]):
    ...
    def _lig_atom_is_hbdonor(self, lig_atom_index, candidate_neighbors_indices):
        ...
        if lig_atom_index in self.lig_atom_is_hbdonor_dict.keys():
            ...
            pos_atom = self.lig_all_atoms_xyz[0][lig_atom_index]
            for candi_neighb_index in candidate_neighbors_indices:
                if lig_atom_index == candi_neighb_index:
                    continue
                else:
                    if self.lig_all_atoms_ad4_types[candi_neighb_index] == "HD":
                        candi_pos = self.lig_all_atoms_xyz[0][candi_neighb_index]
                        candi_d = torch.sqrt(torch.sum((pos_atom - candi_pos) ** 2))
                        if candi_d <= float(self.covalent_radii_dict[self.lig_all_atoms_ad4_types[lig_atom_index]]) + \
                                float(self.covalent_radii_dict[
                                    self.lig_all_atoms_ad4_types[candi_neighb_index]]):
                            the_lig_atom_is_hbdonor = True
    ...
    def update_heavy_atoms_xs_types(self):
        ...
        # clear previous to avoid duplicate appends on repeated calls
        self.updated_lig_heavy_atoms_xs_types = []
    ...
    def generate_frame_heavy_atoms_matrix(self):
        ...
        Returns:
            ...
            # shape (N, N), N is the number of heavy atoms
        """
        N = len(self.lig_heavy_atoms_ad4_types)
        if N == 0:
            self.frame_heavy_atoms_matrix = torch.zeros((0,0), dtype=torch.uint8)
            return self

        mat = torch.zeros((N, N), dtype=torch.uint8)

        # root pairs — use vectorized indexing if possible
        if self.root_heavy_atom_index:
            idx = torch.tensor(self.root_heavy_atom_index, dtype=torch.long)
            mat[idx[:, None], idx[None, :]] = 1

        # other frames
        for heavy_atoms_list in self.frame_heavy_atoms_index_list:
            if not heavy_atoms_list:
                continue
            idx = torch.tensor(heavy_atoms_list, dtype=torch.long)
            mat[idx[:, None], idx[None, :]] = 1

        self.frame_heavy_atoms_matrix = mat
        return self
