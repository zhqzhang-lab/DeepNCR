...
class Optimize(object):
    ...
    def cal_energy(self):
        ...
        pred_rmsd, vina, rmsd_vina, combined_score, pred_ratio = score.process()
        

        return pred_rmsd, vina, rmsd_vina, combined_score, pred_ratio
    ...
    def run(self):
        ...
        for step in range(self.epochs):
            ...
            # scoring
            pred_rmsd, vina_score, rmsd_vina, combined_score, pred_ratio = self.cal_energy()
            ...
            _score_data = np.c_[
                vina_score.detach().numpy(), pred_rmsd.detach().numpy(), rmsd_vina.detach().numpy(), pred_ratio.detach().numpy()]
            ...
            if step == 69:
                ...
                pass
            pred_ratio_loss = ((pred_ratio - 1.0) ** 2).mean()
            pred_ratio_mean = pred_ratio.mean().item()
            if pred_ratio_mean < 0.9:
                alpha = 1.0
                beta = 0.1
            else:
                alpha = 0.5
                beta = 1.0
            loss = alpha * pred_ratio_loss + beta * combined_score
            ...
            self.cnfr = self.cnfr.detach().requires_grad_()
