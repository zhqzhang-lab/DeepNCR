...
def save_data(scores_data, out_dpath, ligand):
    ...
    for num, f in enumerate(poses_fpath):
        ...
        df = pd.DataFrame(_this_poses_values, index=index, columns=['vina', 'pred_rmsd', 'rmsd+vina','ratio_vina','ratio'])
        df.to_csv(f)
...
def save_results(_condition, out_dpath, ligand, scores_data):
    ...
    for index in _update_index:
        ...
        df = pd.DataFrame(_values, index=['init', 'final'], columns=['vina', 'pred_rmsd', 'rmsd+vina','pred_ratio','ratio'])
